...
export default function GridWidget({ reviews, settings }: GridWidgetProps) {
  const gridCols = settings.itemsPerPage || 3; // Default to 3 columns
    ...
    <div className={`w-full p-4 ${settings.theme === 'dark' ? 'bg-gray-800 text-white' : 'bg-white text-gray-900'}`}>
      <div className={`grid grid-cols-1 md:grid-cols-${gridCols} gap-4`}>
        {reviews.map((review) => (
          <div 
            key={review.id}
            className={`p-4 rounded-lg transition-shadow hover:shadow-lg ${
